  ...
  useEffect(() => {
    ...
    if (existingEvent) {
      ...
      reset({
        ...
        instagramEmbed: existingEvent.instagramEmbed || '',
      })

      // Set date/time values
      setDate(new Date(existingEvent.date))
      const [startHour, startMinute] = existingEvent.startTime.split(':').map(Number)
      const [endHour, endMinute] = existingEvent.endTime.split(':').map(Number)
      ...
      setEndTime(endDate)
      setRsvpDeadline(new Date(existingEvent.rsvpDeadline))

      // Set other values
      setHighlights(existingEvent.highlights.length > 0 ? existingEvent.highlights : [''])
      setTags(existingEvent.tags?.length ? existingEvent.tags : [''])
      setFeatured(existingEvent.featured)
      setIsPublished(existingEvent.isPublished)
    } else {
      router.push('/admin/events')
    }
  }, [eventId, getEvent, reset, router])
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Current attendees: {event.attendees}</p>
            </div>
