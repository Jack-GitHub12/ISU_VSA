...
export default function DeckBuilderPage() {
  ...
  const sampleCards = [
    { id: 1, name: 'Lightning Tower', type: 'tower', cost: 3, rarity: 'common', description: 'Basic defensive tower with electrical attacks' },
    { id: 2, name: 'Fireball', type: 'spell', cost: 2, rarity: 'common', description: 'Deal damage to target area' },
    { id: 3, name: 'Guardian Spirit', type: 'unit', cost: 4, rarity: 'rare', description: 'Protective unit with high health' },
    { id: 4, name: 'Resource Boost', type: 'support', cost: 1, rarity: 'common', description: 'Increases resource generation' }
  ]
                ...
                <div className="card">
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg mb-4">
                    <div className="flex justify-between text-sm mb-2">
                      <span>Cards: {currentDeck.totalCards}/{currentDeck.maxCards}</span>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        ...
                        style={{ width: `${(currentDeck.totalCards / currentDeck.maxCards) * 100}%` }}
                      ></div>
                    </div>
                  </div>
                    ...
                    {currentDeck.cards.map((deckCard, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="w-8 text-center text-sm font-semibold">{deckCard.quantity}</span>
                          <button className="w-6 h-6 bg-cardinal text-white rounded-full flex items-center justify-center text-xs hover:bg-cardinal-dark">
      ...
      <section className="py-16 px-4 bg-cream">
        <div className="max-w-7xl mx-auto">
          <AnimatedSection direction="up" className="text-center mb-12">
            ...
            <p className="section-subtitle">Learn from top players and discover winning combinations</p>
          </AnimatedSection>
            ...
            {[
              ...
              >
                <div className="card text-center">
                  <div className={`w-16 h-16 bg-gradient-to-br ${strategy.color} rounded-full flex items-center justify-center mx-auto mb-4`}>
                    <Star className="w-8 h-8 text-white" />
                  </div>
                  <h3 className="text-lg font-bold mb-2">{strategy.name}</h3>
                  <p className="text-gray-600 mb-4">{strategy.description}</p>
                  <div className="flex justify-between text-sm mb-4">
                    <span>Win Rate: <strong>{strategy.winRate}</strong></span>
                    <span>Difficulty: <strong>{strategy.difficulty}</strong></span>
                  </div>
